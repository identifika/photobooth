...
const WEB_URL = process.env.NEXT_PUBLIC_APP_URL || 'https://pika.identifika.my.id';
      ...
      <div style={{ width: '100%', maxWidth: 400 }}>
        ...
        <div style={{ background: 'var(--surface-2)', border: '0.5px solid var(--border)', borderRadius: 12, padding: 24 }}>
          <h2 className="font-medium text-lg text-center mb-5" style={{ color: 'var(--text-primary)' }}>
            {IS_TAURI ? 'Welcome back' : (mode === 'login' ? 'Welcome back' : 'Create your account')}
          </h2>

          {/* ── TAURI / DESKTOP: login only ──────────────────────── */}
          {IS_TAURI ? (
            <>
              ...
              {/* Register message */}
              <div className="text-center mt-5" style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
                Don&apos;t have an account?{' '}
                <span style={{ display: 'block', marginTop: 4, lineHeight: 1.5 }}>
                  For now, you can only register through the website.{' '}
                  <a href={WEB_URL} target="_blank" rel="noopener noreferrer" style={{ fontWeight: 600, color: 'var(--brand)', textDecoration: 'underline' }}>
                    Visit {WEB_URL.replace('https://', '')}
                  </a>
                </span>
              </div>
            </>
